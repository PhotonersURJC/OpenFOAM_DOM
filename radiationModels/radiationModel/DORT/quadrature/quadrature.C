...
bool Foam::radiation::quadrature::calculate()
{
    ...
    forAll(IRay_, rayI)
    {
        ...
        scalar maxRayResidual = IRay_[rayI].correct();	//Solves ray
	...
	if(serialQuad_)
	{
		QrLambda_[actualBand_]=futQrLambda_[actualBand_]+convQrLambda_[actualBand_];
...
void Foam::radiation::quadrature::updateG()
{
	if(serialQuad_)
	{
		updateBandG(actualBand_);
	}
	else
	{
...
void Foam::radiation::quadrature::updateBandG(const label lambdaI)
{
