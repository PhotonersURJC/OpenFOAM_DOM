...
void Foam::radiation::sourceGreyRadiationMixedFvPatchScalarField::initialise()
{
	...
	if(name.find_first_of("Lambda")==1)
	{
		...
		label rayId = Foam::readLabel(IStringStream(name.substr(i2+1, i3-1))());
		if(rayId==0)
		{
			label lambdaId = Foam::readLabel(IStringStream(name.substr(i3+1, name.size()-1))());
			//Information is sent to quadrature: booolean to inform if it has specular reflection, and patchId
			const newRadiationModel& radiation =
			db().lookupObject<newRadiationModel>("radiationProperties");
			const DORT& dort(refCast<const DORT>(radiation));
			label quadIndex = -1;
			dort.setRayModelIndex(internalField().name(), quadIndex);
	
			quadrature& quad = const_cast<quadrature&>(dort.quadratureById(quadIndex));
...
{									
	...
	const discreteOrdinate& ray = dort.quadratureById(quadIndex).IRay(rayId);
	...
	const vector& myRayId = quad.IRay(rayId).d();
